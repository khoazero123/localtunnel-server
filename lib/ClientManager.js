...
class ClientManager {
    ...
    async newClient(id, secret_token) {
        ...
        if (clients[id]) {
            ...
            if(secret_token) {
              if(secret_token == client.secret_token) {
                this.debug("match secret_token: %s", secret_token);
                this.removeClient(id);
              } else {
                this.debug("wrong secret_token: %s != %s", secret_token, client.secret_token);
              }
            } else {
              id = hri.random();
            }
